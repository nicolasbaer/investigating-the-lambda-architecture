...
# This $SCRIPT will download, setup, start, and stop services of the architecture to run on the local machine.
# Please note: mongodb is os dependent and you might have to change the download link
...
DOWNLOAD_KAFKA=https://archive.apache.org/dist/kafka/0.8.1/kafka_2.10-0.8.1.tgz
...
DOWNLOAD_CASSANDRA=http://mirror.switch.ch/mirror/apache/dist/cassandra/2.0.9/apache-cassandra-2.0.9-bin.tar.gz
DOWNLOAD_FLUME=http://archive.apache.org/dist/flume/1.4.0/apache-flume-1.4.0-bin.tar.gz
...
DOWNLOAD_JETTY=http://eclipse.mirror.kangaroot.net/jetty/stable-9/dist/jetty-distribution-9.2.2.v20140723.tar.gz
...
install_yarn() {
  ...
  cp $CONF_DIR/yarn-site.xml $DEPLOY_ROOT_DIR/$SYSTEM/etc/hadoop/yarn-site.xml
}

install_kafka() {
  install "kafka_2.10-0.8.1"
...
install_storm() {
  ...
  cp $CONF_DIR/storm.yaml $DEPLOY_ROOT_DIR/$SYSTEM/conf/storm.yaml
}

install_cassandra() {
  install "apache-cassandra-2.0.9"
  cp $CONF_DIR/cassandra.yaml $DEPLOY_ROOT_DIR/$SYSTEM/conf/cassandra.yaml
}
...
install_mongodb() {
  echo "Unfortunately you'll have to install mongodb yourself, since there's no direct download link. Just download the current mongodb release and uncompress it into the deploy folder."
}

install_logging(){
    install "apache-flume-1.4.0-bin" "flume"
    cp $CONF_DIR/flume.conf $DEPLOY_ROOT_DIR/flume/conf/example.conf

    install "elasticsearch-1.0.1" "elasticsearch"
    cp $CONF_DIR/elasticsearch.yml $DEPLOY_ROOT_DIR/elasticsearch/config/elasticsearch.yml

    cp $DEPLOY_ROOT_DIR/elasticsearch/lib/elasticsearch-* $DEPLOY_ROOT_DIR/flume/lib/
    rm $DEPLOY_ROOT_DIR/flume/lib/lucene-core-*
    cp $DEPLOY_ROOT_DIR/elasticsearch/lib/lucene-core-* $DEPLOY_ROOT_DIR/flume/lib/
    cp $BASE_DIR/flume-ng-elasticsearch-serializer-num-1.0-SNAPSHOT-jar-with-dependencies.jar $DEPLOY_ROOT_DIR/flume/lib


    install "jetty-distribution-9.2.2.v20140723" "jetty"
    install "kibana-3.0.0milestone5" "kibana"
    mv $DEPLOY_ROOT_DIR/kibana $DEPLOY_ROOT_DIR/jetty/webapps/
    #mv $CONF_DIR/kibana.js $DEPLOY_ROOT_DIR/jetty/webapps/kibana/config.js
}
...
if [ "$COMMAND" == "bootstrap" ] && test -z "$SYSTEM"; then
  ...
  echo "  $ grid.sh install [yarn|kafka|zookeeper|storm|redis|mongodb|logging|all]"
  echo "  $ grid.sh start [yarn|kafka|zookeeper|storm|redis|mongodb|logging|all]"
  echo "  $ grid.sh stop [yarn|kafka|zookeeper|storm|redis|mongodb|logging|all]"
  echo
  exit 1
else
