...
import ch.uzh.ddis.thesis.lambda_architecture.speed.spout.kafka.KafkaSpout.EmitState;
import ch.uzh.ddis.thesis.lambda_architecture.speed.spout.kafka.KafkaSpout.MessageAndRealOffset;
import ch.uzh.ddis.thesis.lambda_architecture.speed.spout.kafka.trident.MaxMetric;
...
public class PartitionManager {
    ...
    private long lastOffsetToCommit;
    private final Set<Long> offsetsToCommit = new HashSet<>(1000);


    public PartitionManager(DynamicPartitionConnections connections, String topologyInstanceId, ZkState state, Map stormConf, SpoutConfig spoutConfig, Partition id, TimeWindow<TimestampedOffset> timeWindow, IDataFactory dataFactory) {
        _partition = id;
    ...
    public EmitState next(SpoutOutputCollector collector) {
        ...
        // after the message is emitted we'll store a reference to the new start of the time window, if applicable.
        // therefore the ack mechanism knows which offsets to commit. We do not want to commit any offset that is not
        // related to a time window in order to calculate reliable.
        String message = new String(toEmit.msg.payload().array(), StandardCharsets.UTF_8);
        IDataEntry data = dataFactory.makeDataEntryFromCSV(message);
    ...
    public void ack(Long offset) {
        ...
        if(this.offsetsToCommit.contains(offset)){
            commit(offset);
            this.offsetsToCommit.remove(offset);
        }
    ...
    public void commit(long offset) {
        long lastCompletedOffset = offset;
